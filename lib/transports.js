...
function colorizedConsoleTransport(minimumLogLevel) {
    ...
    var formatting = winston_1.default.format.combine(winston_1.default.format.colorize({
        ...
        return info.label + "  " + info.timestamp + "  " + info.level + " : " + info.message + " " + (info.data || info.tags || info.errorMessage || info.errorStack
            ? JSON.stringify({
                data: info.data,
                tags: info.tags,
...
function awsCloudWatchTransport(_a) {
    ...
    if (!awsRegion) {
        throw new errors_1.ParameterLoggerError("[" + transportName + "] 'AwsRegion' is a required input.");
    }
    if (accessKeyId && !secretAccessKey) {
        throw new errors_1.ParameterLoggerError("[" + transportName + "] AWS 'AccessKeyId' is present, however, \n      'SecretAccessKey' is missing. Ensure both inputs are supplied \n      when custom configuring the logger.");
    }
    else if (!accessKeyId && secretAccessKey) {
        throw new errors_1.ParameterLoggerError("[" + transportName + "] AWS 'SecretAccessKey' is present, however, \n      'AccessKeyId' is missing. Ensure both inputs are supplied when \n      custom configuring the logger.");
    }
    else if (accessKeyId && secretAccessKey) {

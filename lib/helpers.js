...
function getTransports(appName, environmentConfigs, enableValidation) {
    ...
    if (environmentTransports.simpleConsole &&
        ...
        environmentTransports.simpleConsole.forEach(function (transport) {
            transports.push(transports_1.simpleConsoleTransport(transport.minimumLogLevel));
        });
    }
    ...
    if (environmentTransports.colorizedConsole &&
        ...
        environmentTransports.colorizedConsole.forEach(function (transport) {
            transports.push(transports_1.colorizedConsoleTransport(transport.minimumLogLevel));
        });
    }
    // AWS CloudWatch
    if (environmentTransports.awsCloudWatch &&
        ...
        environmentTransports.awsCloudWatch.forEach(function (transport) {
            transports.push(transports_1.awsCloudWatchTransport({
    ...
    if (environmentTransports.rawJSONConsole &&
        ...
        environmentTransports.rawJSONConsole.forEach(function (transport) {
            transports.push(transports_1.rawJSONConsoleTransport(transport.minimumLogLevel));
        });
    }
    return transports;
}
...
function validate(environmentConfigs, environmentName) {
    ...
    if (environments.length === 0) {
        throw new errors_1.ConfigurationLoggerError("There is no configuration for the '" + environmentName + "' environment.");
    }
    if (environments.length > 1) {
        throw new errors_1.ConfigurationLoggerError("There are multiple configurations for the '" + environmentName + "' environment.");
    }
    ...
    if ((!environmentTransports.rawJSONConsole ||
        ...
        (!environmentTransports.awsCloudWatch ||
            environmentTransports.awsCloudWatch.length === 0)) {
        throw new errors_1.ConfigurationLoggerError("At least one transport must be configured for the '" + environmentName + "' environment.");
    }
}
